...
export default async function EventoPage({ params }) {
  let evento
  try {
    evento = await getEvento(params.slug)
  } catch { notFound() }
  if (!evento) notFound()
  ...
  const schema = {
    ...
    offers: { '@type': 'Offer', url: f.linkAfiliado, availability: 'https://schema.org/InStock', seller: { '@type': 'Organization', name: 'Direct Network' } },
  }
      ...
      <main>
        ...
        <div style={{maxWidth:'960px',margin:'0 auto',padding:'0 32px 64px',display:'grid',gridTemplateColumns:'340px 1fr',gap:'40px',alignItems:'start'}}>
          ...
          <div style={{position:'sticky',top:'88px'}}>
            <div style={{width:'100%',aspectRatio:'1/1',borderRadius:'12px',overflow:'hidden',background:'#534AB715',border:'1px solid rgba(83,74,183,0.2)',marginBottom:'12px'}}>
              {flyerUrl ? (
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'20px',paddingTop:'20px'}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:'10px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'7px',fontSize:'13px',color:'var(--text-muted)',background:'var(--bg2)',border:'1px solid var(--border)',padding:'7px 14px',borderRadius:'6px'}}>{dataFormatada}</div>
              <div style={{display:'flex',alignItems:'center',gap:'7px',fontSize:'13px',color:'var(--text-muted)',background:'var(--bg2)',border:'1px solid var(--border)',padding:'7px 14px',borderRadius:'6px'}}>{f.cidade}</div>
            ...
            <div style={{background:'rgba(233,30,140,0.05)',border:'1px solid rgba(233,30,140,0.25)',borderRadius:'var(--radius)',padding:'18px 20px',display:'flex',alignItems:'center',gap:'14px'}}>
              <div style={{width:'44px',height:'44px',borderRadius:'8px',background:'rgba(233,30,140,0.12)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="#E91E8C" strokeWidth="1.5" strokeLinecap="round"><path d="M10 3H6a2 2 0 00-2 2v4l8.5 8.5a1.5 1.5 0 002.1 0l4-4a1.5 1.5 0 000-2.1L10 3z"/><circle cx="7.5" cy="7.5" r="1" fill="#E91E8C" stroke="none"/></svg>
              </div>
              <div>
                <div style={{fontFamily:'var(--font-display)',fontSize:'15px',fontWeight:600,marginBottom:'3px'}}>Desconto exclusivo Direct Network</div>
                <div style={{fontSize:'13px',color:'var(--text-muted)'}}>Já aplicado no link — só clicar e comprar</div>
              </div>
            </div>

            <a href={f.linkAfiliado} target="_blank" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'10px',width:'100%',background:'var(--pink)',color:'#fff',fontFamily:'var(--font-display)',fontSize:'15px',fontWeight:600,padding:'16px',borderRadius:'8px',letterSpacing:'0.01em'}}>
            ...
            <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'var(--radius)',overflow:'hidden'}}>
              {[['Data',dataFormatada],['Local',f.local],['Cidade',f.cidade],['Gênero',f.genero]].map(([label,value]) => (
                <div key={label} style={{display:'flex',alignItems:'center',gap:'14px',padding:'14px 16px',borderBottom:'1px solid var(--border)'}}>
                  ...
                  <div>
                    <div style={{fontSize:'11px',color:'var(--text-faint)',marginBottom:'2px'}}>{label}</div>
