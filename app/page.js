        ...
        <div style={{margin:'0 var(--px) 48px',background:'#0f0a00',border:'1px solid rgba(200,150,60,0.3)',borderRadius:'16px',overflow:'hidden'}}>
          <div className="club-grid" style={{padding:'28px'}}>
            <div>
              ...
              <p style={{fontSize:'14px',color:'var(--text-muted)',lineHeight:1.65,marginBottom:'16px',maxWidth:'520px'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'16px',flexWrap:'wrap'}}>
                ...
                <span style={{fontSize:'13px',color:'var(--text-faint)'}}>A partir de <strong style={{color:'#C8963C'}}>R$&nbsp;48/mês</strong></span>
              </div>
            </div>
            <div className="club-plans">
              {[{name:'Anual',price:'R$ 48',sub:'R$ 474 à vista',featured:true},{name:'Semestral',price:'R$ 56',sub:'R$ 299 à vista'},{name:'Mensal',price:'R$ 68',sub:''}].map(p => (
                <div key={p.name} style={{background:p.featured?'rgba(200,150,60,0.06)':'rgba(255,255,255,0.03)',border:`1px solid ${p.featured?'rgba(200,150,60,0.4)':'rgba(200,150,60,0.12)'}`,borderRadius:'10px',padding:'14px 18px',textAlign:'center',minWidth:'140px',position:'relative'}}>
                  ...
                  <div style={{fontSize:'11px',color:'var(--text-faint)',marginBottom:'4px',textTransform:'uppercase',letterSpacing:'0.06em'}}>{p.name}</div>
                  <div style={{fontFamily:'var(--font-display)',fontSize:'20px',fontWeight:700,color:'#fff'}}>{p.price}<span style={{fontSize:'12px',fontWeight:400,color:'var(--text-faint)'}}>/mês</span></div>
                  {p.sub && <div style={{fontSize:'11px',color:'var(--text-faint)',marginTop:'2px'}}>{p.sub}</div>}
                </div>
              ))}
            </div>
          </div>
