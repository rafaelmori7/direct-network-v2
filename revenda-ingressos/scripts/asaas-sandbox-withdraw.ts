...
// Saque da subconta por Pix para chave CPF ou CNPJ, com a chave DA SUBCONTA (lida de arquivo, nunca impressa).
// Uso: tsx scripts/asaas-sandbox-withdraw.ts <arquivo-da-chave> [cpf-ou-cnpj] [centavos] [--cancelar]
...
async function main() {
  const [file, pixKey = "99991111140", cents] = process.argv.slice(2).filter((a) => !a.startsWith("--"));
  ...
  const result = await provider.withdrawToPix(apiKey, {
    ...
    pixKey,
    pixKeyType: pixKey.replace(/\D/g, "").length === 14 ? "CNPJ" : "CPF",
